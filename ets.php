...
/**
 * ETS - Easy Template System - 3.05c
 * Copyright (C) 2002, 2003  Franck Marcia <phpets@hotmail.com>
 ...
 *
 * $Id: ets.php,v 1.15 2004/03/05 09:28:46 fmarcia Exp $
 */
...
class _ets
{
	...
	function read_content()
	{
		if ($this->external_source_read) {
			return ets_source_read_handler($this->container);
		} else {
			$content = FALSE;
	...
	function read_container($container, $parse)
	{
		...
		if ($parse != _ETS_TEXT) {
			...
			if ($this->external_cache_read && $this->external_cache_write) {
				...
				if ($envelope = ets_cache_read_handler($this->container)) {
				...
				ets_cache_write_handler($this->container, "ETS\1" . serialize($masktree));
				return $masktree;
    ...
    /**
     * Contructor: build the template tree
     */
	function _ets($containers)
	{
		$this->external_source_read = function_exists('ets_source_read_handler');
		$this->external_cache_read  = function_exists('ets_cache_read_handler');
		$this->external_cache_write = function_exists('ets_cache_write_handler');
 ...
 */
function sprintt($datatree, $containers, $entry = 'main')
{
